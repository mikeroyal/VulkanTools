...
ViaSystem::ViaResults ViaSystemMacOS::PrintSystemHardwareInfo() {
    ...
    if (0 == statvfs("/System/Library/CoreServices/SystemVersion", &fs_stats)) {
...
ViaSystem::ViaResults ViaSystemMacOS::PrintRuntimesInFolder(std::string &folder_loc, std::string &object_name, bool print_header) {
    ...
    if (NULL != runtime_dir) {
        ...
        while ((cur_ent = readdir(runtime_dir)) != NULL) {
            // Since this code is a tweeked copy of via_system_linux.cpp, changed magic number 14 to magic number 17.
            // 17 is just the length of the string 'libvulkan.1.dylib'.
            if (NULL != strstr(cur_ent->d_name, object_name.c_str()) && strlen(cur_ent->d_name) == 17) {
                // Get the source of this symbolic link
                command_str = "stat -f%N \'";
        ...
        if (!file_found) {
            PrintBeginTableRow();
            PrintTableElement("");
            PrintTableElement("No libvulkan.dylib files found");
            PrintTableElement("");
            PrintEndTableRow();
        }
    ...
    if (len != -1) {
        ...
        if (pfp == NULL) {
            ...
            while (fgets(path, sizeof(path) - 1, pfp) != NULL) {
                if (NULL != strstr(path, vulkan_dylib_prefix)) {
                    std::string cmd = path;
                    size_t arrow_loc = cmd.find("=>");
                    if (arrow_loc == std::string::npos) {
                        std::string trimmed_path = TrimWhitespace(path, " \t\n\r\'\"");
